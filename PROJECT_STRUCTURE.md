...
### 게임 UI 다국어 지원 확대 (2024)
- **가디언 디펜스 다국어 지원 완료**: 모든 UI 텍스트(게임 제목, 버튼, 모달, 타워 정보, 카드 정보)를 한국어, 영어, 홍콩 번체로 지원
- **이모지 서바이버 다국어 지원 완료**: 모든 UI 텍스트(게임 제목, 모달, 레벨업 메시지, 게임오버 메시지, 무기 정보)를 한국어, 영어, 홍콩 번체로 지원
- 게임 내 하드코딩된 텍스트를 `getUIText()` 함수로 교체하여 동적 번역 구현
- 언어 변경 시 게임 UI가 자동으로 업데이트되도록 `i18n:loaded` 이벤트 리스너 추가
